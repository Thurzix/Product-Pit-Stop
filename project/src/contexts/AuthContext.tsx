...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "addDirectMessage",
      handleAddDirectMessage as EventListener
    );
  ...
  const signup = async (
    ...
    try {
      ...
      // Tentar fazer parse do JSON, com fallback para texto
      let data;
      const contentType = response.headers.get("content-type");
      if (contentType && contentType.includes("application/json")) {
        data = await response.json();
      } else {
        // Se não for JSON, criar um objeto com a mensagem de texto
        const text = await response.text();
        data = {
          success: false,
          message: text || "Erro desconhecido",
          error: response.status === 429 ? "TOO_MANY_REQUESTS" : "UNKNOWN_ERROR"
        };
      }
